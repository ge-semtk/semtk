...
function SparqlServerInterface(serverType, serverURL, graph) {

	// give base URL for the fuseki and the graph parameter.
	this.serverType = serverType;
	this.serverURL = serverURL;
	this.graph = graph;

	// Fuseki and Virtuoso put the /graph and /sparql in different orders in the URL!
	if (serverType == SparqlServerInterface.FUSEKI_SERVER) {
		this.queryURL = encodeURI(this.serverURL) +"/" +  encodeURIComponent(this.graph) + "/sparql?output=json";

	} else if (serverType == SparqlServerInterface.NEPTUNE_SERVER) {
		this.queryURL = encodeURI(this.serverURL) + "/sparql?default-graph-uri=" + encodeURIComponent(graph) + "&format=json";

	} else if (serverType == SparqlServerInterface.VIRTUOSO_SERVER) {
		this.queryURL = encodeURI(this.serverURL) + "/sparql?default-graph-uri=" + encodeURIComponent(graph);
	}
};

SparqlServerInterface.prototype = {
	equals : function (other) {
		return(other.serverType == this.serverType && other.serverURL == this.serverURL && other.graph == this.graph);
	},
	...
	executeQueryAJAXPost : function(sparql, callback, optTimeout, optResultsType) {
			...
			$.ajax({
		        ...
		        type	 : "POST",
		        dataType : "json",
		        data	 : { "query" : sparql, "default-graph-uri" : me.graph, "format" :  me.getFormatValue(resultsType)},
		        timeout  : timeout,
		        statusCode: {
		        	404: function() {
		        		alert( "Status code 404");   // never happens
...
//************** there seems to be a size limit (8K?) on the response, with no error message.  Consider using AJAX when expecting large responses ****************/
	...
	toString : function() {
		return 	this.serverType + " server on " + this.serverURL + ", graph='" + this.graph + "'";
