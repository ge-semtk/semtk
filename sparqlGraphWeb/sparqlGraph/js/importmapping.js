...
define([	// properly require.config'ed
	...
	function(MappingItem) {
		...
		ImportMapping.prototype = {
			...
			/**
			Is the mapping a type_map , not a normal node or property
			 */
			isTypeRestriction: function() {
				return this.propItem.getURI() == ImportMapping.TYPE_URI;
			},
			
			// jNode - json
			// idHash
			// ng
			// opt_type_flag - this is a 'type_map', not a normal node or prop
			fromJsonNode: function(jNode, idHash, ng, optTypeParentNode) {
				if (!jNode.hasOwnProperty("sparqlID") && !optTypeParentNode) { kdlLogAndThrow("Internal error in ImportMapping.fromJsonNode().  No sparqlID field."); }
			...
			toJson: function(idHash) {
				...
				} else if (this.propItem.getURI() == ImportMapping.TYPE_URI) {
					// nothing
				} else {
					// prop row
