		...
		MappingTab.prototype = {
			...
			draw : function(highlightColor="") {
				...
				for (var i=0; i < colList.length; i++) {
                    var colName = colList[i].getColName();
                    var but = this.buildColValidatorButton(colName, this.importSpec.getDataValidator().get(colName));
            ...
            buildColValidatorButton : function(colName, colValidator) {
                var iconName = (colValidator != null && !colValidator.isEmpty()) ? "icon-check" : "icon-check-empty";

                var but = IIDXHelper.createIconButton(iconName, function(){}, ["btn", "btn-small"]);
                var callback = this.launchModalValidateCol.bind(this, colName, but);
                but.onclick = callback;
                return but;
            },

            launchModalValidateCol :  function(colName, iconButton) {
                var dialog = new ModalValidateColumnDialog(
                                    this.importSpec.getDataValidator().get(colName),
                                    colName,
                                    this.validateColumnModalCallback.bind(this, colName, iconButton));
                dialog.launch();
            },

            validateColumnModalCallback : function(colName, iconButton, colValidator) {
                // save the colValidator
                this.importSpec.getDataValidator().put(colValidator);

                // update the button
                var newBut = this.buildColValidatorButton(colName, colValidator);
                iconButton.parentNode.replaceChild(newBut, iconButton);
            },
