 ...
 */
	 ...
	 *  (2) Register inside the beforeUpdatingCallback
...
define([	// properly require.config'ed
	...
	function() {
		...
		var HtmlFormGroup = function(document, queryClient, domain, g_fields, g_query, statusFunc, alertFunc, beforeUpdatingCallback, doneUpdatingCallback, optDefaultHash, optGetFlag) {
			...
			this.query = null;
			this.fields = null;
			this.beforeUpdatingCallback = beforeUpdatingCallback;
		...
		HtmlFormGroup.prototype = {
			...
			getNodegroup : function(callback) {			
			...
			buildNodeGroup : function () {
				...
				for (var i = 0; i < this.fields.length; i++) {
					...
					snode = this.nodeGroup.getOrAddNode(uri, this.oInfo, this.domain, true, pathOptionalFlag); // superClassFlag=true				
			...
			addQueryReturns : function (nodeGroup, optSkipConstraintFlag) {
				...
				for (var i = 0; i < retlist.length; i++) {
					...
					snode = nodeGroup.getOrAddNode(uri, this.oInfo, this.domain, true, optFlag);   // superClassFlag=true
			...
			runIndependentElemValuesQuery : function (element) {
				...
				this.queryClient.executeAndParse(sparql, (function(res){this.callbackSetChoices(res);}).bind(this));
			}, 
			...
			fieldChanged : function(elementId, optUpdateCountFlag) {
				...
				// call the beforeUpdatingCallback before doing anything
				if (this.beforeUpdatingCallback) {
					this.beforeUpdatingCallback();
				}
			...
			pruneNodeGroup : function(nodeGroup) {
				...
				for (var i=0; i < this.query.dontPrune.length; i++) {
					...
					// search for uri
					var snodes = nodeGroup.getNodesBySuperclassURI(uri, this.oInfo);
					if (snodes.length == 0) {
						...
						// temporarily set to isReturned
						var snode = snodes[0];
						saveRet.push(snode.getIsReturned());
						saveNode.push(snode);
						snode.setIsReturned(true);
					}
				}
				...
				for (var i=0; i < saveNode.length; i++) {
					saveNode[i].setIsReturned(saveRet[i]);
				}
			},
		...
		return HtmlFormGroup;            // return the constructor
	}
);
