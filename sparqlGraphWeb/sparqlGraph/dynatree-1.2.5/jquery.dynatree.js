/*! ****************************************************************************
	NOTE: Code added for search function by Paul Cuddihy
	      Search down for "stackoverflow" to find it.
...
*/
	...
	// Code for search is Added in from:
	// http://stackoverflow.com/questions/12277797/jquery-dynatree-search-node-by-name
	// Modified with "toLowerCase()" in two locations
	//
	// Uses a style as such:
	// <style>
	...
	find: function (needle) {
	    ...
	    if (needle.length >= 1) {
	        var occurs = [];
	        this.visit(function (node) {
	            //$(node.span).removeClass('occurance'); //remove previous findings
	        	node.select(false);      // PEC changed to select the node instead
	        	
	            if (node.data.title.toLowerCase().indexOf(needle) != -1) {
	                occurs.push(node);
	                node._expandPath();
	            }
	        });
	        for (indx in occurs) { // mark findings
	            ...
	            occurs[indx].select(true);  // PEC changed to select the node instead
	        }
