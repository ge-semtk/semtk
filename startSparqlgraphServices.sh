...
PORT_SPARQL_NODE_GROUP_STORE_SERVICE=12058
...
$JAVA_HOME/bin/java -jar $SEMTK/ontologyInfoService/target/ontologyInfoService-1.1.0-SNAPSHOT.jar --spring.config.location=$SEMTK/ontologyInfoService/src/main/resources/ontologyinfo.properties --server.port=$PORT_ONTOLOGY_INFO_SERVICE --multipart.maxFileSize=1000Mb > $LOGS/ontologyInfoService.log 2>&1 &

$JAVA_HOME/bin/java -jar $SEMTK/nodeGroupStoreService/target/nodeGroupStoreService-1.1.0-SNAPSHOT.jar --spring.config.location=$SEMTK/nodeGroupStoreService/src/main/resources/store.properties --server.port=$PORT_SPARQL_NODE_GROUP_STORE_SERVICE --multipart.maxFileSize=1000Mb > $LOGS/nodeGroupStoreService.log 2>&1 &
