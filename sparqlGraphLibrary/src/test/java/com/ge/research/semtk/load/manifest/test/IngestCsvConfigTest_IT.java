...
public class IngestCsvConfigTest_IT extends YamlConfigTest{
	...
	public void test() throws Exception{
		...
		try {
			...
			IngestCsvConfig ingestCsvConfigWithNoDatagraphInYaml = new IngestCsvConfig(Utility.getResourceAsFile(this, "/manifest/IngestionPackage/TestData/Package-1/import-withNoDatagraph.yaml"), modelFallbackSei.getGraph(), dataFallbackSei.getGraph());
			...
			SparqlEndpointInterface dataSeiFromYaml = TestGraph.getSei(ingestCsvConfig.getDatagraphs().get(0));		// this is what's in the YAML
			...
			clearGraphs();
			ingestOwlConfig.load(null, TestGraph.getSparqlServer(), TestGraph.getSparqlServerType(), new PrintWriter(System.out));  // loads OWL to fallback
			ingestCsvConfigWithNoDatagraphInYaml.load(null, null, TestGraph.getSparqlServer(), TestGraph.getSparqlServerType(), false, IntegrationTestUtility.getIngestorRestClient(), IntegrationTestUtility.getNodeGroupExecutionRestClient(), new PrintWriter(System.out));
			assertEquals(dataSei.getNumTriples(), 0);
