...
public class DataLoaderTest_IT {
	...
	public void testPrecheckOnly() throws Exception {
		...
		assertEquals(0, dl.getTotalRecordsProcessed()); 
		
		// check that results are empty
		Table tab = TestGraph.execTableSelect(sgJson);
		assertEquals(0, tab.getNumRows());
	}
	...
	public void testLookupCreatePartial() throws Exception {
		...
		Table t1 = TestGraph.execTableSelect(sgJson);
		
		// Weird note: this query is sorted on an column that is a result of an optional
		// MarkLogic fills empties with nulls which sort to the end
		// Others (e.g. Fuseki) fill with "" which sort to the beginning
		// So queryAndCheckResults() fails
		// but sortByAllCols() then compareResults() works out.
		t1.sortByAllCols();
		IntegrationTestUtility.compareResults(t1.toCSVString(), this, "/loadTestLookupCreatePartialResults.csv");
