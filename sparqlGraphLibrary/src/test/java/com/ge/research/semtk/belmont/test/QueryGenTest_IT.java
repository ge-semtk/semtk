...
public class QueryGenTest_IT {
	...
	@Test
	public void testQueryGenFail() throws Exception {

		// cyclic path
		NodeGroup nodegroup = TestGraph.getNodeGroup("src/test/resources/animalQueryCyclicPath.json"); 
		try{
			nodegroup.generateSparql(AutoGeneratedQueryTypes.QUERY_DISTINCT, false, 100, null); 
			fail("Nodegroup contains cyclic path but no exception was thrown");
		}catch(Exception e){
			System.out.println("Exception as expected:" + e.getMessage());
			assert(e.getMessage().contains("SemTK does not support cyclic paths"));
		}

		// add other failure modes here
	}	
