...
public class NodeGroupResultSetTest {
	...
	public void test1() {
		try{
			...
			String constructQueryResultJSON = "{ \"@graph\": [ " 
					...
					+ "] }";		
			JSONObject nodeGroupJSON = (JSONObject) (new JSONParser()).parse(constructQueryResultJSON);

			// create a NodeGroupResultSet
			NodeGroupResultSet ngrs = new NodeGroupResultSet(true);
			ngrs.addResultsJSON(nodeGroupJSON);

			// retrieve the node group
			NodeGroup ng = ngrs.getResults();			
			
			// test copied from LoadNodeGroupFromConstructQueryResults test
