...
public class IngestOwlConfigTest extends YamlConfigTest {
	...
	public void test() throws Exception{
		...
		config = new IngestOwlConfig(TestGraph.getJunitYaml(this, "/manifest/ingest_owl_config_1.yaml"), modelFallbackSei.getGraph());
		...
		config = new IngestOwlConfig(TestGraph.getJunitYaml(this, "/manifest/ingest_owl_config_2.yaml"), modelFallbackSei.getGraph());
		assertEquals(config.getFiles().size(), 3);
		assertEquals(config.getModelgraph(),"http://junit/animals/model");

		// this config has model graph as string
		config = new IngestOwlConfig(TestGraph.getJunitYaml(this, "/manifest/ingest_owl_config_3.yaml"), modelFallbackSei.getGraph());
		assertEquals(config.getFiles().size(), 3);
		assertEquals(config.getModelgraph(),"http://junit/animals/model");

		// this config has multiple model graphs.  Legacy schema supports this (likely unused), disallowing it here
		try {
			config = new IngestOwlConfig(TestGraph.getJunitYaml(this, "/manifest/ingest_owl_config_4.yaml"), modelFallbackSei.getGraph());
			fail();
		}catch(Exception e) {
