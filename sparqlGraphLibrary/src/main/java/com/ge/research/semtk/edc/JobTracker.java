...
public class JobTracker {
	 ...
	 * NOTE: called by status service at startup
	 * @param tracker
	 * @param retrySec
	 * @throws Exception
	 */
	public static void uploadOwlModel(JobTracker tracker, int retrySec) throws Exception {
		...
		while (true) {
			try {
				...
				checkedOwl.add(key);
				// check owl, potentially uploading without telling oinfo service to uncache.  
				// ok since job tracker uses custom sparql, not nodegroups.
				StartupUtilities.updateOwlIfNeeded(tracker.createSuperuserEndpoint(), null, JobTracker.class, "/semantics/OwlModels/serviceJob.owl");
				return;
			} catch (Exception e) {
