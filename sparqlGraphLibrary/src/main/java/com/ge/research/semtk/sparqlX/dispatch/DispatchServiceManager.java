...
public class DispatchServiceManager {
	...
	private void calcServiceMnemonic() throws Exception {
		...
		ArrayList<String> valClasses = oInfo.getSuperclassNames(valURI);
		valClasses.add(valURI);
		
		ArrayList<String> srcClasses = null;
		if (srcURI != null) {
	...
	private void edcNodegroupCheckRestrictions() throws BadQueryException, Exception {
		...
		for (int i=0; i < classURI.length; i++) {
			...
			Returnable item = snode;
			
			int limit = 0; 
			int operandI = Integer.parseInt(operand[i]);
			
			// limit the count query to one greater than the operator for efficiency
			// because after that the answer will always be the same anyway
			limit = operandI + 1;
			
			// run the query
			String sparql = tmpNodegroup.generateSparql(AutoGeneratedQueryTypes.QUERY_COUNT, false, limit, item);
			TableResultSet res = (TableResultSet) this.extConfigSei.executeQueryAndBuildResultSet(sparql, SparqlResultTypes.TABLE);
			
			// check the results
			if (res.getSuccess()) {
				Table table = res.getResults();
				if (table.getNumRows() == 1 && table.getNumColumns() == 1) {
					
					// got a valid single-number count query result.  Now do the real check.
					int count = Integer.parseInt(table.getRows().get(0).get(0));
					...
					if (operator[i].equals("==")) {
						...
						if (count != operandI) {
							String msg = String.format("Query must return exactly %d values for %s.  %d or more found.", intOperand, shortURI, count);
						...
						if (count > operandI) {
							String msg = String.format("Query must return at most %d values for %s.  %d or more found.", intOperand, shortURI, count);
							throw new BadQueryException(msg);
						}
						...
						if (count < operandI) {
							String msg = String.format("Query must return at least %d values for %s.  %d found.", intOperand, shortURI, count);
							throw new BadQueryException(msg);
						}
					}
				} else {
					throw new Exception ("Internal error: nodegroup check count query returned multiple values.");
				}
			} else {
				throw new Exception ("Internal error: nodegroup check count query. " + res.getRationaleAsString("\n"));
			}
		}
	}
