	...
	private String generateSparqlSubgraphClauses(AutoGeneratedQueryTypes queryType, Node snode, NodeItem skipNodeItem, Node skipNodeTarget, Returnable targetObj, ArrayList<Node> doneNodes, String tab) throws Exception  {
		...
		for(NodeItem nItem : snode.getNodeItemList()) {
			...
			for(Node targetNode : nItem.getNodeList()) {
				...
				if (nItem != skipNodeItem || targetNode != skipNodeTarget) {
					...
					// open optional block
					if(nItem.getSNodeOptional(targetNode) == NodeItem.OPTIONAL_TRUE && nItem.getNodeList().size() > 0 && queryType != AutoGeneratedQueryTypes.QUERY_CONSTRUCT && queryType != AutoGeneratedQueryTypes.QUERY_CONSTRUCT_FOR_INSTANCE_MANIPULATION){
						sparql.append(tab + "optional {\n");
						tab = tabIndent(tab);
						blockFlag = true;
					} else if(nItem.getSNodeOptional(targetNode) == NodeItem.MINUS_TRUE && nItem.getNodeList().size() > 0 && queryType != AutoGeneratedQueryTypes.QUERY_CONSTRUCT && queryType != AutoGeneratedQueryTypes.QUERY_CONSTRUCT_FOR_INSTANCE_MANIPULATION){
					...
					sparql.append(tab + snode.getSparqlID() + " " + this.applyPrefixing(nItem.getUriConnectBy()) + " " + targetNode.getSparqlID() + " .\n");
					tab = tabIndent(tab);
		...
		for(NodeItem nItem : this.getConnectingNodeItems(snode)) {   
			...
			if (nItem != skipNodeItem || snode != skipNodeTarget) {
				...
				// open optional
				if (nItem.getSNodeOptional(snode) == NodeItem.OPTIONAL_REVERSE && nItem.getNodeList().size() > 0  && queryType != AutoGeneratedQueryTypes.QUERY_CONSTRUCT && queryType != AutoGeneratedQueryTypes.QUERY_CONSTRUCT_FOR_INSTANCE_MANIPULATION) {
					sparql.append(tab + "optional {\n");
					tab = tabIndent(tab);
					blockFlag = true;
				} else if (nItem.getSNodeOptional(snode) == NodeItem.MINUS_REVERSE && nItem.getNodeList().size() > 0  && queryType != AutoGeneratedQueryTypes.QUERY_CONSTRUCT && queryType != AutoGeneratedQueryTypes.QUERY_CONSTRUCT_FOR_INSTANCE_MANIPULATION) {
					sparql.append(tab + "minus {\n");
					tab = tabIndent(tab);
					blockFlag = true;
				}
				...
				if (incomingSNode != null) {
					...
					sparql.append(tab + incomingSNode.getSparqlID() + " " + this.applyPrefixing(nItem.getUriConnectBy()) + " " + snode.getSparqlID() + " .\n");
					//tab = tabIndent(tab);
				}
		...
		if(constrainedTypes.size() == 0 || !constrainedTypes.contains(curr.getFullUriName() )){
			...
			if(curr.getSubClassNames().size() == 0){
				...
				if(queryType != AutoGeneratedQueryTypes.QUERY_CONSTRUCT && queryType != AutoGeneratedQueryTypes.QUERY_CONSTRUCT_FOR_INSTANCE_MANIPULATION){	// not valid in the Query leading section.
					retval += tab + curr.getSparqlID() +  "_type " + " rdfs:subClassOf* " + this.applyPrefixing(curr.getFullUriName()) + ".\n";
				}
			}
		}
...
//	}
			...
			if (! skipNodes.contains(fromSnode)) {
				...
				for (NodeItem nodeItem : fromSnode.getNodeItemList()) {
					...
					for (Node toSnode : nodeItem.getNodeList()) {
						...
						if (nodeItem.getSNodeOptional(toSnode) == NodeItem.OPTIONAL_REVERSE || 
								nodeItem.getSNodeOptional(toSnode) == NodeItem.MINUS_REVERSE ) {
	...
	public String generateSparqlDelete(String post, OntologyInfo oInfo) throws Exception {
		...
		retval.append(this.generateSparqlWithClause("") + "\n");
