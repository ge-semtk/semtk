...
public class NodeGroup {
	...
	private String generateSparqlSubgraphClausesNodeItem(AutoGeneratedQueryTypes queryType, boolean incomingFlag, Node snode, NodeItem nItem, Node targetNode, Returnable targetObj, ArrayList<Node> doneNodes, ArrayList<Integer> doneUnions, String tab) throws Exception {
		...
		HashSet<String> subPropNames = (this.oInfo == null) ? new HashSet<String>() : this.oInfo.getSubPropNames(nItem.getUriConnectBy(), snode.getUri());
		...
		if (subPropNames.size() == 0) {
			...
			if (queryType == AutoGeneratedQueryTypes.QUERY_CONSTRUCT) {
				...
				sparql.append(tab + this.genSubPropertiesContraint(varName, nItem.getUriConnectBy(), subPropNames) + ".\n");
	...
	private String generateSparqlSubgraphClausesPropItem(AutoGeneratedQueryTypes queryType, Node snode, PropertyItem prop, Returnable targetObj, String tab) throws Exception {
		...
		HashSet<String> subPropNames = (this.oInfo == null) ? new HashSet<String>() : this.oInfo.getSubPropNames(prop.getUriRelationship(), snode.getUri());
		if (subPropNames.size() == 0) {
			// regular prop
			...
			if (queryType == AutoGeneratedQueryTypes.QUERY_CONSTRUCT) {
				...
				sparql.append(tab + snode.getSparqlID() + " " + varName + " " + prop.getSparqlID() + " .\n");
				sparql.append(tab + this.genSubPropertiesContraint(varName, prop.getUriRelationship(), subPropNames) + ".\n");
	...
	private String generateSparqlTypeClause(Node node, String tab, AutoGeneratedQueryTypes queryType) throws Exception  {
		...
		if(constrainedTypes.size() == 0 || !constrainedTypes.contains(node.getFullUriName() ) || 
				queryType == AutoGeneratedQueryTypes.QUERY_CONSTRUCT  || queryType == AutoGeneratedQueryTypes.QUERY_CONSTRUCT_WHERE){
			...
			if (this.oInfo == null) {
				...
				// as long as we're not in the top of a CONSTRUCT 
				if(queryType != AutoGeneratedQueryTypes.QUERY_CONSTRUCT){	
				...
				// as long as we're not in the top of a CONSTRUCT 
				if(queryType != AutoGeneratedQueryTypes.QUERY_CONSTRUCT){	// not valid in the Construct Query leading section.
	...
	private String genDeletionLeader(String post, OntologyInfo oInfo) throws Exception {
		...
		for(Node n : this.nodes){
			...
			for( NodeItem ni : n.getNodeItemList() ) {
				...
				HashSet<String> subPropNames = (this.oInfo == null) ? new HashSet<String>() : this.oInfo.getSubPropNames(ni.getUriConnectBy(), n.getUri());
