...
public class NodeGroup {
	...
	private String generateSparqlSubgraphClausesNodeItem(AutoGeneratedQueryTypes queryType, boolean incomingFlag, Node snode, NodeItem nItem, Node targetNode, Returnable targetObj, ArrayList<Node> doneNodes, ArrayList<Integer> doneUnions, String tab) throws Exception {
		...
		if (this.oInfo == null || ! this.oInfo.hasSubProperties(nItem.getUriConnectBy())) {
			...
			String varName = SparqlToXUtils.safeSparqlVar(snode.getSparqlID() + "_" + nItem.getUriConnectBy()+ " " + targetNode.getSparqlID());
			
			if (queryType == AutoGeneratedQueryTypes.QUERY_CONSTRUCT) {
				// variable w/o qualifier
				sparql.append(tab + snode.getSparqlID() + " " + varName + " " + targetNode.getSparqlID() + " .\n");

			} else if (	queryType == AutoGeneratedQueryTypes.QUERY_CONSTRUCT_WHERE ||
					   	queryType == AutoGeneratedQueryTypes.QUERY_DELETE_WHERE) {
				// variable with qualifier
				predStr = this.applyQualifier(varName, nItem.getQualifier(targetNode));
				sparql.append(tab + snode.getSparqlID() + " " + predStr + " " + targetNode.getSparqlID() + " .\n");
				
				// constraint on the props variable
				sparql.append(tab + this.genSubPropertiesContraint(varName, snode.getUri(), nItem.getUriConnectBy()) + ".\n");
				
			} else {
				// (this|that) with qualifier
				predStr = this.genSubPropertyListParenthesized(nItem.getUriConnectBy(), snode.getUri()); 
				predStr = this.applyQualifier(predStr, nItem.getQualifier(targetNode));
				sparql.append(tab + snode.getSparqlID() + " " + predStr + " " + targetNode.getSparqlID() + " .\n");
			}
		}
	 ...
	 * @throws Exception
	 */
	private String genSubPropertiesContraint(String varName, String nodeUri, String propUri) throws Exception {
		ArrayList<String> propNames = new ArrayList<String>();
		propNames.add(propUri);
		propNames.addAll(this.oInfo.getSubPropNames(propUri, nodeUri)); 
	 ...
	 * @throws Exception
	 */
	private String genSubPropertyListParenthesized(String nodeUri, String propUri) throws Exception {
		StringBuilder propStr = new StringBuilder();
		propStr.append("(" + this.applyPrefixing(propUri));
		for (String subProp : this.oInfo.getSubPropNames(propUri, nodeUri)) {
	...
	private String generateSparqlSubgraphClausesPropItem(AutoGeneratedQueryTypes queryType, Node snode, PropertyItem prop, Returnable targetObj, String tab) throws Exception {
		...
		if (this.oInfo == null || ! this.oInfo.hasSubProperties(prop.getUriRelationship())) {
			...
			String varName = SparqlToXUtils.safeSparqlVar(snode.getSparqlID() + "_" + prop.getUriRelationship());

			if (queryType == AutoGeneratedQueryTypes.QUERY_CONSTRUCT) {
				// varname no values clause
				sparql.append(tab + snode.getSparqlID() + " " + varName + " " + prop.getSparqlID() + " .\n");

			} else if (	queryType == AutoGeneratedQueryTypes.QUERY_CONSTRUCT_WHERE ||
				   		queryType == AutoGeneratedQueryTypes.QUERY_DELETE_WHERE) {
				// varname plus values clause
				sparql.append(tab + snode.getSparqlID() + " " + varName + " " + prop.getSparqlID() + " .\n");
				sparql.append(tab + this.genSubPropertiesContraint(varName, snode.getUri(), prop.getUriRelationship()) + ".\n");
				
			} else {
				// "normal": parenthesized predicate
				sparql.append(tab + snode.getSparqlID() + " " + this.genSubPropertyListParenthesized(snode.getUri(), prop.getUriRelationship())+  " " + prop.getSparqlID() +  " .\n");
			}
	...
	private String generateSparqlTypeClause(Node curr, String tab, AutoGeneratedQueryTypes queryType) throws Exception  {
		...
		if(constrainedTypes.size() == 0 || !constrainedTypes.contains(curr.getFullUriName() )){
			...
			if (this.oInfo != null && this.oInfo.getSubclassNames(curr.getFullUriName()).size() == 0) {
				...
				if(queryType != AutoGeneratedQueryTypes.QUERY_CONSTRUCT){	// not valid in the Query leading section.
		...
		for(Node n : this.nodes){
			...
			for( PropertyItem pi : n.getPropertyItems() ){
				if(pi.getIsMarkedForDeletion()){
					if (this.oInfo == null || ! this.oInfo.hasSubProperties(pi.getUriRelationship())) {
			...
			for( NodeItem ni : n.getNodeItemList() ) {
				...
				for( Node connected : nic ){
					if (this.oInfo == null || ! this.oInfo.hasSubProperties(ni.getUriConnectBy())) {
