...
public class NodeGroup {
	...
	/**
	 * Check nodegroup for conditions that generate syntactically correct or empty sparql
	 * but that will fail.
	 * Helps for UI where we don't want to throw errors during nodegroup-building and get too many dialogs.
	 * Meant for incomplete nodegroups.
	 * @param qt
	 * @return message or null
	 */
	public String checkNodegroup(AutoGeneratedQueryTypes qt) {
		switch (qt) {
		case QUERY_DISTINCT:
			if (this.getReturnedSparqlIDs().size() == 0) {
				return "Nothing selected to return.";
			}
			if (this.getFunctionSparqlIDs().size() > 0 && this.groupBy.size() < this.getReturnedSparqlIDs(true).size()) {
				
				ArrayList<String> needGrouping = new ArrayList<String>();
				for (String id : this.getReturnedSparqlIDs(true)) {
					if (! this.groupBy.contains(id)) {
						needGrouping.add(id);
					}
				}
				return "Returns missing from GROUP BY: " + String.join(", ", needGrouping);
			}
			break;
		...
		case QUERY_DELETE_WHERE:
			if (this.countItemsForDeletion() == 0) {
				return "Nothing marked for deletion";
			}
			break;
		}
	...
	private String generateGroupByClause() throws Exception {
		this.validateOrderBy();
