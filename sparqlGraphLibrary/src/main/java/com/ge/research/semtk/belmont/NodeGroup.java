...
public class NodeGroup {
	...
	public String generateSparql(AutoGeneratedQueryTypes qt, Boolean allPropertiesOptional, Integer limit, Returnable targetObj) throws Exception{
		return this.generateSparql(qt, allPropertiesOptional, limit, targetObj, false);
	}

	public String generateSparql(AutoGeneratedQueryTypes qt, Boolean allPropertiesOptional, Integer limit, Returnable targetObj, Boolean keepTargetConstraints) throws Exception{
		//
		// queryType:
	...
	public String generateSparqlAsk() throws Exception{
		...
		Node headNode = this.getNextHeadNode(doneNodes);
		while (headNode != null) {
			footer += this.generateSparqlSubgraphClauses(AutoGeneratedQueryTypes.QUERY_CONSTRUCT_WHERE, headNode, null, null, doneNodes, tab);
			headNode = this.getNextHeadNode(doneNodes);
		}
...
//			sparql.append(tab + snode.getSparqlID() + " <" + prop.getUriRelationship() + "> " + prop.getSparqlID() +  " .\n");
		...
		for(NodeItem nItem : snode.getNodeItemList()) {
			...
			if (nItem != skipNodeItem) {
				...
				if(nItem.getIsOptional() == NodeItem.OPTIONAL_TRUE && nItem.getNodeList().size() > 0 && queryType != AutoGeneratedQueryTypes.QUERY_CONSTRUCT){
					sparql.append(tab + "optional {\n");
					tab = tabIndent(tab);
				}
				
				// each nItem might point to multiple children
				for(Node domainNode : nItem.getNodeList()) {
					sparql.append("\n");
					...
					// RECURSION
					sparql.append(this.generateSparqlSubgraphClauses(queryType, domainNode, nItem, targetObj, doneNodes, tab));
					tab = tabOutdent(tab);
				}
		...
		for(NodeItem nItem : this.getConnectingNodeItems(snode)) {   // PEC TODO: write this

			if (nItem != skipNodeItem) {
			
				// open optional
				if (nItem.getIsOptional() == NodeItem.OPTIONAL_REVERSE && nItem.getNodeList().size() > 0  && queryType != AutoGeneratedQueryTypes.QUERY_CONSTRUCT) {
					sparql.append(tab + "optional {\n");
					tab = tabIndent(tab);
				}
...
//					sparql.append(tab + incomingSNode.getSparqlID() + " <" + nItem.getUriConnectBy() + "> " + snode.getSparqlID() + " .\n");
				...
				// RECURSION
				sparql.append(this.generateSparqlSubgraphClauses(queryType, incomingSNode, nItem, targetObj, doneNodes, tab));
				tab = tabOutdent(tab);
	...
	public void expandOptionalSubgraphs() {
		...
		for (Node snode : this.nodes) {
			...
			if (optRet > 0 && nonOptRet == 0) {		
				...
				if (connectedSnodes.size() == 1) {
					...
					if (nodeItems.size() == 1 && nodeItems.get(0).getIsOptional() == NodeItem.OPTIONAL_FALSE) {
						NodeItem nodeItem = nodeItems.get(0);						
						
						// make the nodeItem optional inward
						if (snode.getNodeItemList().contains(nodeItem)) {
		...
		while (changedFlag) {
			...
			for (Node snode : this.nodes) {
				...
				ArrayList<NodeItem> connItems = this.getAllConnectedNodeItems(snode);
				for (NodeItem nItem : connItems) {
					if (nItem.getConnected()) {	
						int opt = nItem.getIsOptional();
						
						if (opt == NodeItem.OPTIONAL_FALSE) {
							normItems.add(nItem);
							
						} else if (snode.getNodeItemList().contains(nItem)) {
				...
				if (nonOptReturnCount == 0 && normItems.size() == 1 && optOutItems.size() > 0 && optInItems.size() == 0) {
				
					// set the single normal nodeItem to incoming optional
					NodeItem n = normItems.get(0);
					if (snode.getNodeItemList().contains(n)) {
	...
	private HashMap<String, Integer>  calcOptionalHash(ArrayList<Node> skipNodes) {
		...
		for (Node snode : this.nodes) {
			...
			if (! skipNodes.contains(snode)) {
				...
				for (NodeItem nodeItem : snode.getNodeItemList()) {
					...
					int opt = nodeItem.getIsOptional();
					if (opt != NodeItem.OPTIONAL_FALSE) {
						ArrayList<Node> subGraph = new ArrayList<Node>();
						// get subGraph(s) on the optional side of the nodeItem
						if (opt == NodeItem.OPTIONAL_TRUE) {
							for (Node j : nodeItem.getNodeList()) {
								ArrayList<Node> stopList = new ArrayList<Node>();
								stopList.add(snode);
								subGraph.addAll(this.getSubGraph(j, stopList));
							}
	...
	private ArrayList<String> getConnectedRange(Node node) {
		...
		for (NodeItem ni : nodeItems) {
			if (ni.getIsOptional() != NodeItem.OPTIONAL_REVERSE) {
