...
public class ImportSpecHandler {
		...
		if (cachedUri != null) {
			...
			if (tab.getNumRows() > 1) {
				...
				if (this.getLookupMode(nodeID).equals(ImportSpec.LOOKUP_MODE_NO_CREATE)) {
					// this is the only spot where we didn't run query but need lookupNodegroup for error msg
	...
	private String getLookupQuery(NodeGroup lookupNodegroup, String nodeID, ArrayList<String> builtStrings) throws Exception {
		...
		return lookupNodegroup.generateSparql(AutoGeneratedQueryTypes.QUERY_DISTINCT, false, 0, null);
	}
