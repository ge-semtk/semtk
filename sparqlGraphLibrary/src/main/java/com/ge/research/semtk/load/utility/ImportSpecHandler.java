...
public class ImportSpecHandler {
	...
	/**
	 * Calculate unused and missing columns in the dataset.
	 * It is often a feature to allow extra columns so users don't have to slice-and-dice CSV files,
	 * but when a column is misspelled accidentally it is also nice to get a message.
	 * 
	 * Note that columns can be required due to URI lookups or the DataValidator.  
	 * Those errors are generated independently.
	 * 
	 * Warnings generated here are intended to be informational only.
	 * 
	 * @param dsHeaders - column headers in the incoming data set
	 * @return - a possibly empty ArrayList<String> of warning messages
	 */
	public ArrayList<String> generateColumnWarnings(ArrayList<String> dsHeaders) {
		
		ArrayList<String> warnings = new ArrayList<String>();
		ArrayList<String> missingCols = new ArrayList<String>();
		ArrayList<String> unusedCols = new ArrayList<String>();
		Set<String> colsUsedInSpec = this.colsUsed.keySet();
		ArrayList<String> standardizedDsCols = new ArrayList<String>();
		
		// lower-case the columns
		for (String dsHeader : dsHeaders) {
			standardizedDsCols.add(dsHeader.toLowerCase());
		}
		
		// missing columns: in the import spec but not in the input data
		for (String specCol : colsUsedInSpec) {
			if (! standardizedDsCols.contains(specCol)) {
				missingCols.add(specCol);
			}
		}
		if (missingCols.size() > 0) {
			warnings.add("Input is missing these columns: " + StringUtils.join(missingCols, ", "));
		}
		
		// unused columns : in dataset but not in the import spec
		for (String dsCol : standardizedDsCols) {
			if (! colsUsedInSpec.contains(dsCol)) {
				unusedCols.add(dsCol);
			}
		}
		if (unusedCols.size() > 0) {
			warnings.add("Input contains unused columns: " + StringUtils.join(unusedCols, ", "));
		}
