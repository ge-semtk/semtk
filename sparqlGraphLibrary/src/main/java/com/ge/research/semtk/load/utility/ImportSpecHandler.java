...
public class ImportSpecHandler {
	...
	private void preFetchUriCache_SAVE(String nodeID) throws Exception {

		/**
		 * Paul   2/5/21
		 * 
		 * With optimized URI Lookup queries and/or blazegraph, this probably isn't needed.
		 * It is also a tricky thing to figure out how many to lookup up before memory runs out.  (That's what a database is for !!!)
		 * 
		 * 
		 * Saving this code for just a litte while.
		 * 
		 */
		...
		// loop through lookupMappings: remove constraints.  set sparqlIDs: map0, map1, ...
		int i = 0;
		for (ImportMapping mapping : this.lookupMappings.get(nodeID)) {
			...
			if (mapping.isNode()) {
				...
				lookupNodegroup.changeSparqlID(node, "map"+ String.valueOf(i));
				lookupNodegroup.setIsReturned(node, true);
				node.setValueConstraint(null);  

			} else {
				PropertyItem prop = node.getPropertyByURIRelation(mapping.getPropURI());
				lookupNodegroup.changeSparqlID(prop, "map"+ String.valueOf(i));
				lookupNodegroup.setIsReturned(prop, true);
		...
		// efficiently verify the column numbers of each builtString:  map0, map1, ...
		int mappingCount = this.lookupMappings.get(nodeID).size();
		int colNum[] = new int[mappingCount];
		for (int m=0; m < mappingCount; m++) {
			String colName = "map"+ String.valueOf(m);
			colNum[m] = tab.getColumnIndex(colName);
		}
		int uriCol = tab.getColumnIndex(nodeID);
		
		// loop through rows returned
		for (ArrayList<String> row : tab.getRows()) {
			
			// build the builtStrings
			ArrayList<String> builtStrings = new ArrayList<String>();
			for (int m=0; m < mappingCount; m++) {
				builtStrings.add(row.get(colNum[m]));
			}
			
			this.uriCache.putIfNewElseDelete(this.lookupNodegroupMD5.get(nodeID), builtStrings, row.get(uriCol));
		}
		
		// REMAINING TASKS:
		// only one thread should be able to pre-fetch the cache
		// re-do change so that builtStrings (used to save uriCache) are updated at the TOP of lookupUri so e.g. the enums "RED" and "http://full/uri#RED" are cached the same
		// decide how may rows to cache, and what to do when it's full  <<<< DIFFICULT
		// use limit/offset to cache a different set
		// use a variable to know where we are in this process (including done)
		// build the builtStrings and add these to the cache
		// sort the returns by map in SPARQL and skip the last one that != next - to make sure we don't add first instance of a duplicate.
		//       sorting could crash Virtuoso if super large number.
	...
	private long lookupCount(String nodeID, int maxCount) throws Exception {
		...
		String query = lookupNodegroup.generateSparql(AutoGeneratedQueryTypes.QUERY_COUNT, false, maxCount, null);
