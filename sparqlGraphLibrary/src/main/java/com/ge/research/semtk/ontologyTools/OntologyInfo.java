...
public class OntologyInfo {
	...
	private static String getLoadPropertiesQuery(SparqlConnection conn, String domain) throws Exception {
		String retVal = ""
				...
				+ "         VALUES ?prop_type { owl:ObjectProperty owl:DatatypeProperty } . ?Property a ?prop_type . \n"
				...
				+ "         VALUES ?prop_type { owl:ObjectProperty owl:DatatypeProperty rdf:Property } . ?Property a ?prop_type . \n"
				...
				+ "\n"
				+ "		VALUES ?class_pred { owl:onClass owl:allValuesFrom owl:someValuesFrom } ."
				+ "		# range\n"
				+ " 	{      \n"
				+ "			# restriction has simple range or allValuesFrom or someValuesFrom a non-blank node\n"
				+ "			?rest ?class_pred ?Range " + genDomainFilterStatement("Range", domain, "|| regex(str(?Range),'XML')") + ". \n"
				+ "		} UNION {\n"
...
//				+ "?x owl:allValuesFrom ?Range " + genDomainFilterStatement("Range", domain, "|| regex(str(?Range),'XML')") + ". \n" 
	...
	private boolean ngHasInstance(NodeGroup ngTemp, Node nodeAdded, SparqlConnection conn) throws Exception {
		...
		int savedTimeout = sei.getSparqlTimeout();

		try {
			int newTimeout = this.pathFindingMaxTimeMsec / 2000;
			sei.setSparqlTimeout(newTimeout);
			if (CONSOLE_LOG) {
			...
			sei.setSparqlTimeout(savedTimeout);
		}
