...
import com.ge.research.semtk.services.nodeGroupExecution.requests.ConstructInstanceWithPredicatesRequest;
...
import com.ge.research.semtk.services.nodeGroupExecution.requests.StitchByIdRequestBody;
...
public class NodeGroupExecutionRestController {
	...
	public JSONObject constructConnectedData(@RequestBody ConstructConnectedDataRequest requestBody, @RequestHeader HttpHeaders headers) {
		...
		try {	
			...
			ConnectedDataConstructor constructor = new ConnectedDataConstructor(
					...
					requestBody.getClassList(), requestBody.getIsListWhite(), requestBody.getIsListSuperclasses(),
	...
	@Operation(
			summary="CONSTRUCT a single instance with a given set of predicates",
			description="result has 'JobId' field"
			)
	@CrossOrigin
	@RequestMapping(value="/constructInstanceWithPredicates", method=RequestMethod.POST)
	public JSONObject constructInstanceWithPredicates(@RequestBody ConstructInstanceWithPredicatesRequest requestBody, @RequestHeader HttpHeaders headers) {
		final String ENDPOINT_NAME="constructInstanceWithPredicates";
		HeadersManager.setHeaders(headers);
		...
		try {
			...
			ng.constrainNodeToInstance(node, requestBody.getInstanceUri());	// constrain to given URI
			for(String predicate : requestBody.getPredicateList()) {		// connect given predicates
				PropertyItem pItem = node.getPropertyByURIRelation(predicate);
				if(pItem != null) {
					pItem.setOptMinus(PropertyItem.OPT_MINUS_OPTIONAL);
					ng.setIsReturned(pItem, true);
				}
				NodeItem nItem = node.getNodeItem(predicate);
				if(nItem != null) {
					for(String range : nItem.getRangeUris()) {
						Node n = ng.addNode(range, node, null, nItem.getUriConnectBy());
						nItem.setOptionalMinus(n, NodeItem.OPTIONAL_TRUE);
					}
				}
			}
			
			// dispatch the nodegroup
			DispatchFromNodegroupRequestBody dispatchRequestBody = new DispatchFromNodegroupRequestBody(); 
			dispatchRequestBody.setSparqlConnection(conn.toString());
			dispatchRequestBody.setJsonRenderedNodeGroup(ng.toJson().toString());
			return dispatchAnyJobFromNodegroup(dispatchRequestBody, AutoGeneratedQueryTypes.CONSTRUCT, SparqlResultTypes.N_TRIPLES);
	...
	@CrossOrigin
	@RequestMapping(value="/dispatchStitchedNodegroups", method= RequestMethod.POST)
	public JSONObject dispatchStitchedNodegroups(@RequestBody StitchByIdRequestBody requestBody, @RequestHeader HttpHeaders headers) {
		HeadersManager.setHeaders(headers);
		try {
			SimpleResultSet res = new SimpleResultSet(true);
			String jobId = JobTracker.generateJobId();			
			...
			SimpleResultSet err = new SimpleResultSet(false);
			err.addRationaleMessage(SERVICE_NAME, "dispatchStitchedNodegroups", e);
			LocalLogger.printStackTrace(e);
			return err.toJson();
		}
	}
